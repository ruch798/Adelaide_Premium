...
import com.example.adelaidepremium.interfaces.CustomerInterface;
import com.example.adelaidepremium.interfaces.OrderInterface;
import com.example.adelaidepremium.interfaces.ProductInterface;
import com.example.adelaidepremium.interfaces.SalesInterface;
...
/***
 * Sales Controller: To Enter Order
 */
public class SalesController
    implements Initializable, ProductInterface, OrderInterface, SalesInterface, CustomerInterface {

  @FXML private TableView<Product> productTableView, productTableView1;
  ...
  @FXML private TextField searchField, searchField2;

  @FXML private TableView<Customer> customerTableView;

  @FXML private TableColumn<Customer, String> customerNameColumn, customerCityColumn;
  @FXML private TableColumn<Customer, Integer> customerIDColumn, customerZipColumn;
  ...
  @FXML private ObservableList<Order> ORDERLIST;
  private OrderModel orderModel;

  @FXML private ObservableList<Sales> SALESLIST;
  private SalesModel salesModel;

  @FXML private TextField customerIDField, productIDField, quantityField, paymentField;
  ...
  private void filterCustomersData() {
    ...
    searchField2.setOnKeyReleased(
        s -> {
          searchField2
              .textProperty()
              .addListener(
                  (observable, oldValue, newValue) -> {
                    searchedCustomerData.setPredicate(
                        customer -> {
                          if (newValue == null || newValue.isEmpty()) {
                            return true;
                          }
    ...
    try {
      if (validateProductInput()) {
        ...
        productModel.decreaseProduct(
            Integer.parseInt(productIDField.getText()), Integer.parseInt(quantityField.getText()));

        Product p1 = productModel.getProductById(Integer.parseInt(productIDField.getText()));

        //        set price from product list
        ...
        //        set price from discounted product list
        for (Product p3 : PRODUCTLIST_DISCOUNTED) {
          if (p3.getId() == p1.getId()) {
            p1.setPrice(p3.getPrice());
          }
        }
        ...
        alert.setContentText("The product has been added to the cart successfully!");
        alert.showAndWait();
      }
    } catch (NumberFormatException ex) {
      System.out.println(ex.getMessage() + ", please enter appropriate values");
  ...
  protected void confirmOrderButtonClick() {
    ...
    try {
      if (validateOrderInput()) {
        ...
        //        For payment just simulate payment by asking the user for credit card details and
   ...
   *
   * @param isSimulated
   * @return order date and delivery date contained in an array called dates
   */
